...
// console.log("test :", _pokemon[1]);

// let fromInputData = "이";

// console.log(typeof(_pokemon))
...
// 첫번째 글자가 "이"라고 시작하는 포켓몬은 몇마리인가
// 1. 임의의 글자 선정 -> 첫글자 "조회" -> 있다면 카운트 ->목록 배열
...
//   if(_pokemon[i].startsWith(firstLetter)) {
...
let pokeArray = []
for(let i = 0; i < _pokemon.length; i++) {
  if(_pokemon[i][0] === firstLetter) {
    // console.log(_pokemon[i]);
    pokeArray[countHandler] = _pokemon[i];
    countHandler = countHandler + 1
  } 
}
...
// let lastLetter = '뇽';
// for(let i = 0; i < _pokemon.length; i++) {
//   if(_pokemon[i].endsWith(lastLetter)) {
//     console.log(_pokemon[i]);
//     countHandler = countHandler + 1
//   } 
// }
